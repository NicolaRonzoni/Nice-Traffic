...
########################################################################################################################
#ramps
df=pd.read_csv(r'PHILIPPE.COMPTAGE.SORTIE.NORD.csv')
df

df['NAME']
#speed
df_flow=df.loc[(df['NAME'] =="PHILIPPE.COMPTAGE.SORTIE.NORD.CPT6")] 
df_flow['RECTS'] = pd.to_datetime(df_flow['RECTS'], errors='coerce')
df_flow['RECTS'] = df_flow['RECTS'].dt.floor('Min')
#delete useless columns 
df_flow=df_flow.drop(columns=['CHRONO','NAME','BATT'])
df_flow

df_flow=df_flow.set_index('RECTS')

df_flow=df_flow.sort_values(by=['RECTS'])

##fill the jupps in the series 

df_flow_6=df_flow.resample('6min').mean()

#### remove the night

df_flow_6=df_flow_6.between_time('5:59', '22:59')

#2019
train=df_flow_6[0:62050]

#20/6-26/6
test_1=df_flow_6[62050+3230:62050+3230+1190]
#24/2-1/3
test_2=df_flow_6[62050+9180:62050+9180+1190]
#23/3-29/3
test_3=df_flow_6[62050+13940:62050+13940+1190]
#20/7-26/7
test_4=df_flow_6[62050+34170:62050+34170+1190] 


df_def=pd.concat([train,test_1,test_2,test_3,test_4])

len(df_def)

df_def=df_def.fillna(-1)

df_def
###### select only the four weeks as test 

df_def.to_excel('/Users/nronzoni/Desktop/Voie Mathis/loop detector/Philippe sortie nord.xlsx')


df=pd.read_excel(r'/Users/nronzoni/Desktop/Voie Mathis/loop detector/Cimiez entree.xlsx')

df=df.set_index('RECTS')
df=df.sum(axis=1)

df.to_excel('/Users/nronzoni/Desktop/Voie Mathis/loop detector/Cimiez entree.xlsx')
##################################################################################################

cimiez_nord=pd.read_excel(r'/Users/nronzoni/Desktop/Voie Mathis/loop detector/Voie Mathis North direction.xlsx', sheet_name=0)

cimiez_nord=cimiez_nord.replace(-1,np.nan)

cimiez_nord.isnull().sum() * 100 / len(cimiez_nord)
cimiez_nord=cimiez_nord.interpolate(limit=30)
cimiez_nord

cimiez_nord.isnull().sum() * 100 / len(cimiez_nord)

philippe_nord=pd.read_excel(r'/Users/nronzoni/Desktop/Voie Mathis/loop detector/Voie Mathis North direction.xlsx', sheet_name=1)

philippe_nord=philippe_nord.replace(-1,np.nan)

philippe_nord.isnull().sum() * 100 / len(philippe_nord)
philippe_nord=philippe_nord.interpolate(limit=30)
philippe_nord

philippe_nord.isnull().sum() * 100 / len(philippe_nord)

magnan=pd.read_excel(r'/Users/nronzoni/Desktop/Voie Mathis/loop detector/Voie Mathis North direction.xlsx', sheet_name=2)

magnan=magnan.replace(-1,np.nan)

magnan.isnull().sum() * 100 / len(magnan)
magnan=magnan.interpolate(limit=30)
magnan

magnan.isnull().sum() * 100 / len(magnan)

fabron_sortie=pd.read_excel(r'/Users/nronzoni/Desktop/Voie Mathis/loop detector/Voie Mathis North direction.xlsx', sheet_name=3)

fabron_sortie=fabron_sortie.replace(-1,np.nan)

fabron_sortie.isnull().sum() * 100 / len(fabron_sortie)

fabron_sortie=fabron_sortie.interpolate(limit=30)
fabron_sortie


fabron_sortie.isnull().sum() * 100 / len(fabron_sortie)

bosquets_entree=pd.read_excel(r'/Users/nronzoni/Desktop/Voie Mathis/loop detector/Voie Mathis North direction.xlsx', sheet_name=4)

bosquets_entree=bosquets_entree.replace(-1,np.nan)

bosquets_entree.isnull().sum() * 100 / len(bosquets_entree)
bosquets_entree=bosquets_entree.interpolate(limit=30)
bosquets_entree

bosquets_entree.isnull().sum() * 100 / len(cimiez_sud)

grinda=pd.read_excel(r'/Users/nronzoni/Desktop/Voie Mathis/loop detector/Voie Mathis North direction.xlsx', sheet_name=5)

grinda=grinda.replace(-1,np.nan)

grinda.isnull().sum() * 100 / len(grinda)
grinda=grinda.interpolate(limit=30)
grinda
...
# Write each dataframe to a different worksheet.
cimiez_nord.to_excel(writer, sheet_name='Cimiez Nord')
philippe_nord.to_excel(writer, sheet_name='Philippe Nord')
magnan.to_excel(writer, sheet_name='Magnan')
fabron_sortie.to_excel(writer, sheet_name='Fabron sortie')
bosquets_entree.to_excel(writer, sheet_name='Bosquets entree')
grinda.to_excel(writer, sheet_name='Grinda')
